...
namespace ChessV
{
	public struct BitBoard
	{
		...
		public BitBoard(int capacity, UInt64 bits0, UInt64 bits1 = 0UL, UInt64 bits2 = 0UL, UInt64 bits3 = 0UL)
		{
			this.bits0 = bits0;
			this.bits1 = bits1;
			this.bits2 = bits2;
		...
		public int LSB
		{
			...
			//	returns least significant bit
			{
				...
				case 3:
					...
					if (bits3 != 0)
						return index64[((bits3 & (UInt64)(-((Int64)bits3))) * debruijn64) >> 58] + 194;
					return -1;
		...
		public int GetBit( int bitnumber )
		{
			...
			{
				case 3:
					return (int)( bits3 >> (bitnumber % 64)) & 1;
		...
		public bool IsBitSet( int bitnumber )
		{
			...
			{
				case 3:
					return ((bits3 >> (bitnumber % 64)) & 1) != 0UL;
		...
		public void SetBit( int bitnumber )
		{
			switch( bitnumber / 64 )
			{
				case 3:
					if (bitCount >= 0)
						bitCount += ((int)(bits3 >> (bitnumber % 64)) & 1) ^ 1;
					bits3 |= 1UL << (bitnumber % 64);
					break;
				case 2:
					if( bitCount >= 0 )
		...
		public void ClearBit( int bitnumber )
		{
			switch( bitnumber / 64 )
			{
				case 3:
					if (bitCount >= 0)
						bitCount -= ((int)(bits3 >> (bitnumber % 64)) & 1);
					bits3 &= 0xFFFFFFFFFFFFFFFFUL ^ (1UL << (bitnumber % 64));
					break;
				case 2:
					if( bitCount >= 0 )
		...
		//	returns and clears the least significant bit
		{
			...
			switch( capacity )
			{
				case 3:
					...
					if (bits3 != 0)
					{
						returnval = index64[((bits3 & (UInt64) (-((Int64)bits3))) * debruijn64) >> 58] + 192;
						bits3 = bits3 & (UInt64)((Int64)bits3 - 1);
						return returnval;
					}
					return -1;
				case 2:
		...
		#region SetData
		public void SetData( UInt64 b0, UInt64 b1, UInt64 b2, UInt64 b3, int bitct = -1 )
		{
			bits0 = b0;
			bits1 = b1;
			bits2 = b2;
		...
		public static implicit operator bool( BitBoard bb )
		{
			return bb.bitCount >= 0
				...
				: ((bb.capacity == 0 &&  bb.bits0 != 0UL) ||
				   ...
				   (bb.capacity == 2 && (bb.bits0 != 0UL || bb.bits1 != 0UL || bb.bits2 != 0UL)) || (bb.capacity == 3 && (bb.bits0 != 0UL || bb.bits1 != 0UL || bb.bits2 != 0UL || bb.bits3 != 0UL)));
		}
		...
		public static BitBoard operator <<( BitBoard bb, int shift )
		{
			switch( bb.capacity )
			{
				...
				case 3:
					return new BitBoard( 3, bb.bits0 << shift, (bb.bits1 << shift) | (bb.bits0 >> (64 - shift)), 
						(bb.bits2 << shift) | (bb.bits1 >> (64 - shift)), (bb.bits3 << shift) | (bb.bits2 >> (64 - shift)));
			}
			throw new Exception( "Unsupported bitboard capacity" );
		}
		...
		public static BitBoard operator >>( BitBoard bb, int shift )
		{
			switch( bb.capacity )
			{
				...
				case 3:
					return new BitBoard(3, (bb.bits0 >> shift) | (bb.bits1 << (64 - shift)), (bb.bits1 >> shift) | (bb.bits2 << (64 - shift)), (bb.bits2 >> shift) | (bb.bits3 << (64 - shift)), bb.bits3 >> shift );
			}
			throw new Exception( "Unsupported bitboard capacity" );
		}
		...
		{
			return new BitBoard( b1.capacity, b1.bits0 & b2.bits0, b1.bits1 & b2.bits1, b1.bits2 & b2.bits2, b1.bits3 & b2.bits3 );
		}
		#endregion
